...
export abstract class AuthService {
   ...
   * @return {null} returns null if failed to fetch user info
   */
...
export abstract class AuthJwtService {
   ...
   */
  abstract generateJwtAccessToken(userId: string): string;
   ...
   */
  abstract verifyJwtAccessToken(token: string): string;
}
